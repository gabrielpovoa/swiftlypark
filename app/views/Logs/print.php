...
<div class="container mx-auto p-6 font-sans text-gray-900">
    <h1 class="text-3xl md:text-4xl font-bold text-center text-[#1E2A47] mb-8">
    ...
    <div class="overflow-x-auto rounded-xl shadow-lg border border-gray-200 bg-white">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr>
                ...
                <th class="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider">Hora Entrada</th>
                <th class="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider">Hora Saída</th>
            ...
            <?php if (!empty($logs)): ?>
                <?php foreach ($logs as $log): ?>
                    <tr class="hover:bg-blue-50 transition duration-150">
                        ...
                        <td class="px-6 py-3 font-medium text-green-600">R$<?= number_format(floatval($log['valor_pago'] ?? 0), 2, ',', '.') ?></td>
    ...
    <div class="flex flex-col md:flex-row justify-between mt-8 gap-4 print:hidden">
        <button onclick="window.print()"
                class="flex items-center justify-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-lg shadow-md hover:bg-blue-700 transition-colors duration-300 font-semibold">
            <i data-lucide="printer" class="w-5 h-5"></i> Imprimir
        </button>

        <a href="/"
           class="flex items-center justify-center gap-2 bg-gray-200 text-gray-800 px-6 py-3 rounded-lg shadow-md hover:bg-gray-300 transition-colors duration-300 font-semibold">
