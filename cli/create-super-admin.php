...
if ($name === '' || $email === '') {
    fwrite(
        STDERR,
        "Uso: php cli/create-super-admin.php --name=\"Super Admin\" --email=\"admin@example.com\"\n"
    );
    exit(1);
}
...
$activeSuperAdmin = $connection->query(
...
if ((int) $activeSuperAdmin > 0) {
    fwrite(STDERR, "Já existe um Super-Admin ativo. Operação abortada.\n");
    exit(1);
}
...
$role = $connection->prepare(
    "SELECT id FROM roles WHERE slug = 'super-admin' AND is_active = 1 LIMIT 1"
);
$role->execute();
$roleId = $role->fetchColumn();

if ($roleId === false) {
    fwrite(STDERR, "Papel super-admin não encontrado. Execute as migrations RBAC antes.\n");
    exit(1);
}
...
echo "Super-Admin inicial criado com id {$userId}. A senha temporária foi adicionada à fila.\n";
